...
def disconnectWithClient(message, client, client_name):
    ...
    if (entered_client_name in clients):
            ...
            greet_message=f"Hello,{entered_client_name} You're successfully disconnected with {client_name}"
            other_client_socket.send(greet_message.encode())
            msg=f"You're successfully disconnected with{entered_client_name}"
...
def connectClient(message, client, client_name):
    ...
    if (entered_client_name in clients):
        if(not clients[client_name]["connected_with"]):
            ...
            other_client_socket.send(greet_message.encode())
            msg=f"You're successfully connected with{entered_client_name}"
            client.send(msg.encode())
        else:
            other_client_name=clients[client_name]["connected_with"]
            msg=f"You're already connected with{other_client_name}"
            client.send(msg.encode())
